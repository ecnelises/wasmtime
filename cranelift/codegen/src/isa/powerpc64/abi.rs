//! Implementation of a PowerPC64 ELF v2 ABI
...
use crate::isa;
use crate::isa::powerpc64::{inst::EmitState, inst::*, settings as powerpc64_settings};
...
// We use a generic implementation that factors out PPC64 and x64 ABI commonalities, because
...
/// Support for the PPC64 ABI from the caller side (at a callsite).
pub(crate) type PPC64CallSite = CallSite<PPC64MachineDeps>;
...
/// PPC64-specific ABI behavior. This struct just serves as an implementation
/// point for the trait; it is never actually instantiated.
pub struct PPC64MachineDeps;

impl IsaFlags for powerpc64_settings::Flags {
...
impl ABIMachineSpec for PPC64MachineDeps {
    ...
    type F = powerpc64_settings::Flags;
        ...
        I: IntoIterator<Item = &'a ir::AbiParam>,
    {
        ...
        let is_apple_cc = call_conv.extends_apple_powerpc64();

        // See PPC64 ABI (https://github.com/ARM-software/abi-aa/blob/2021Q1/aapcs64/aapcs64.rst#64parameter-passing), sections 6.4.
        ...
        for param in params {
            ...
            //
            // See PPC64 ABI (https://github.com/ARM-software/abi-aa/blob/2021Q1/aapcs64/aapcs64.rst#642parameter-passing-rules), (Section 6.4.2 Stage C).
            //
            ...
            if is_multi_reg {
                ...
                if reg_space >= 2 && reg_class_space >= 2 {
                    // The powerpc64 ABI does not allow us to start a split argument
            ...
            let size = if is_apple_cc {
                // MacOS powerpc64 allows stack slots with
    ...
    fn gen_args(_isa_flags: &powerpc64_settings::Flags, args: Vec<ArgPair>) -> Inst {
        Inst::Args { args }
    }

    fn gen_ret(
        setup_frame: bool,
        isa_flags: &powerpc64_settings::Flags,
        call_conv: isa::CallConv,
        rets: Vec<RetPair>,
    ...
    fn gen_prologue_start(
        ...
        match select_api_key(isa_flags, call_conv, setup_frame) {
            ...
            None => {
                ...
                if flags.unwind_info() && call_conv.extends_apple_powerpc64() {
    ...
    fn gen_probestack(_insts: &mut SmallInstVec<Self::I>, _: u32) {
        // TODO: implement if we ever require stack probes on an PPC64 host
        // (unlikely unless Lucet is ported)
        unimplemented!("Stack probing is unimplemented on PPC64");
    }
