//! PowerPC64 ISA: binary code emission.
        ...
        match self {
            ...
            &Inst::VecShiftImm {
                ...
                let immh_immb = match (size.lane_size(), is_shr) {
                    ...
                    _ => panic!(
                        "powerpc64: Inst::VecShiftImm: emit: invalid op/size/imm {:?}, {:?}, {:?}",
                        op, size, imm
                    ),
                };
            ...
            &Inst::LoadExtName {
                ...
                if emit_info.0.is_pic() {
                    ...
                    // adrp rd, symbol
                    sink.add_reloc(Reloc::PPC64AdrGotPage21, name, 0);
                    let inst = Inst::Adrp { rd, off: 0 };
                    inst.emit(&[], sink, emit_info, state);

                    // ldr rd, [rd, :got_lo12:X]
                    sink.add_reloc(Reloc::PPC64Ld64GotLo12Nc, name, 0);
            ...
            &Inst::ElfTlsGetAddr { ref symbol, rd } => {
                ...
                // adrp x0, <label>
                sink.add_reloc(Reloc::PPC64TlsGdAdrPage21, symbol, 0);
                let inst = Inst::Adrp { rd, off: 0 };
                inst.emit(&[], sink, emit_info, state);

                // add x0, x0, <label>
                sink.add_reloc(Reloc::PPC64TlsGdAddLo12Nc, symbol, 0);
            ...
            &Inst::MachOTlsGetAddr { ref symbol, rd } => {
                ...
                sink.add_reloc(Reloc::MachOPPC64TlsAdrPage21, symbol, 0);
                sink.put4(0x90000000);

                // ldr x0, [x0, <label>@TLVPPAGEOFF]
                sink.add_reloc(Reloc::MachOPPC64TlsAdrPageOff12, symbol, 0);
                ...
                Inst::CallInd {
                    info: crate::isa::Box::new(CallIndInfo {
                        ...
                        caller_callconv: CallConv::ApplePPC64,
                        callee_callconv: CallConv::ApplePPC64,
                        callee_pop_size: 0,
                    }),
                }
                .emit(&[], sink, emit_info, state);
